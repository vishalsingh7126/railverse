...
function buildTrainSuggestions(query: string, trains: UnifiedTrain[]): SuggestionItem[] {
  ...
  return trains
    .filter((train) => train.number.toUpperCase().startsWith(normalized))
...
export function TrainSearchForm({ onSearch, onSelectTrain, loading = false }: TrainSearchFormProps) {
  ...
  return (
    <section ref={rootRef} className="glass rounded-2xl p-6 md:p-8">
      <div className="mb-8">
        <h2 className="font-display text-2xl font-bold tracking-tight">Search Trains</h2>
        <p className="mt-2 text-sm text-foreground/70">Find the perfect train for your journey</p>
      </div>

      <div className="space-y-6">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-5 items-end">
          ...
          <div className="flex justify-center lg:pb-7">
            <button
              type="button"
              onClick={handleSwap}
              className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-foreground/15 bg-primary/10 text-primary-soft transition hover:bg-primary/20 hover:border-primary/30"
          ...
          <div className="relative">
            <label className="mb-2 block text-xs font-semibold uppercase text-foreground/60">
